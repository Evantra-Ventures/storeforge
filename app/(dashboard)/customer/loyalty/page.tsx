...
export default function CustomerLoyaltyPage() {
    ...
    const [tenantOptions, setTenantOptions] = useState<LoyaltyTenantOption[]>([]);
    const [selectedTenant, setSelectedTenant] = useState<LoyaltyTenantOption | null>(
        null
    );
...
router.push(`/login?redirect=${encodeURIComponent(currentPath)}`);
            ...
            const resolveTenantForLoyalty = async () => {
                const searchParams =
                    typeof window !== "undefined"
                ...
                let resolvedTenant: LoyaltyTenantOption | null = null;
                ...
                const tenantMap = new Map<string, LoyaltyTenantOption>();
...
setTenantOptions(options);
            ...
            const resolvedTenant = await resolveTenantForLoyalty();

            if (!resolvedTenant) {
                setErrorMessage(
                    "Choose a store first. Your loyalty rewards are tracked separately for each merchant."
                );
                setTenantId(null);
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div>
                        ...
                        <p className="mt-2 text-slate-500">
                            {selectedTenant?.name || "This store"} has not enabled loyalty rewards yet.
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-slate-500">
                                Loyalty points are tracked separately for each merchant.
                            </p>
                        </div>
                        ...
                        <select
                            ...
                            onChange={(event) => {
                                ...
                                if (typeof window !== "undefined") {
                                    window.localStorage.setItem("storeforge:last_loyalty_store", slug);
                                }

                                router.push(`/customer/loyalty?store=${slug}`);
                            }}
                                    ...
                                    >
                                        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                                            <div>
                                                <div className="flex flex-wrap items-center gap-2">
                                                    <span
                                                        className={`rounded-full px-3 py-1 text-xs capitalize ${isNegative
                                            ...
                                            <div className="md:text-right">
                                                <p
                                                    className={`text-lg font-bold ${isNegative ? "text-red-700" : "text-green-700"
                                                        }`}
                                                >
                                                    {isNegative ? "-" : "+"}
                                                    {Math.abs(
